...
- 能用 DOM/URL/日志判断的状态，优先用 Playwright locator、页面文本、URL 跳转和脚本 `RESULT:`；不要靠截图猜流程。
- 必须截图的场景：AI 封面生成效果、二维码、上传后的封面预览、弹窗遮罩、按钮可见但点不动、浏览器被其他窗口挡住或跑到另一个 Space。
- 截图只证明“屏幕上长什么样”，不等于发布成功；最终成功以页面跳转 `home`、成功页、日志 `RESULT: success` 为准。

## 直接请求 / API replay 边界

公众号个人/未认证主体没有稳定发文 API，本 skill 默认走网页自动化。理论上可以对网页后台的私有接口做 API discovery 并重放真实请求，但前提是已经捕获并验证：

- endpoint、method、请求体、referer/origin
- 登录 cookie、token、CSRF、可能的签名/随机参数
- 草稿 id、群发/仅发表状态、扫码验证后的状态机
- 请求后能从后台或成功页回读确认

未做完这些验证前，不要“mock 响应”骗过本地脚本，也不要绕过微信验证。`继续发表` 这类按钮会推进后台状态并触发扫码；在接口未确认前，点击真实 UI 比猜私有请求更稳。
