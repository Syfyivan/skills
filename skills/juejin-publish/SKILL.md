...
掘金文章 = 一个标题 + 一段 **Markdown 正文**(编辑器是 ByteDance 自家的 **Bytemd**，底层 CodeMirror，左写右预览)。发布时弹出抽屉，**分类为必选**，可选标签(3~5 个)/封面/摘要/专栏。本 skill 把一篇 Markdown 文章填进掘金写文章页并(可选)发布。**掘金无对外发文 API**，全程网页自动化。
...
node scripts/juejin_publish.cjs --content-file /abs/article.md --post \
  --category "前端" --tags "React,性能优化" --summary "一句话摘要" --column "我的专栏"
```
...
| `--tags "a,b,c"` | 否 | 逗号分隔；逐个在标签框搜索并选第一个候选(建议 3~5 个) |
| `--summary "..."` | 否 | 摘要，填进抽屉摘要框(截 200 字) |
...
| 标签 | byte-select：`[class*="byte-select__placeholder"]`(「请搜索添加标签」)，选项 `li[class*="byte-select-option"]` | 搜索→选候选 |
| 封面 | `input[type="file"]` | 不传则掘金自动抓正文首图 |
| 摘要 | `textarea[placeholder*="摘要"]` / `textarea[class*="byte-input__textarea"]` | 「编辑摘要」 |
...
- **标签 byte-select**：每个标签 = 点搜索框 → `keyboard.type` → 选第一个含该词的 `byte-select-option`。掘金标签数量有限制(通常 ≤5)，多了会失败。
...
```bash
node ../publish-cli.mjs juejin --file /abs/article.md --title "..." [--category "前端"] [--tags a,b] [--summary "..."] [--column "专栏"]   # 填好不发
